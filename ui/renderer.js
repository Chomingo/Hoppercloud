...
function updateHeaderTitle(name) {
    if (headerUsername) {
        headerUsername.textContent = name && name.trim().length > 0 ? name : 'Safioland Launcher';
    }
}
...
async function loadNews() {
    try {
        ...
        if (process.defaultApp) {
            ...
            const fs = require('fs-extra');
            const localNewsPath = path.join(__dirname, '..', 'launcher_news.json');
            if (fs.existsSync(localNewsPath)) {
                newsData = JSON.parse(fs.readFileSync(localNewsPath, 'utf-8'));
            }
        } else {
            // Packaged: fetch from GitHub
            const response = await fetch('https://raw.githubusercontent.com/Chomingo/Hoppercloud/master/launcher_news.json');
        ...
        if (titleEl && newsData.titulo) titleEl.textContent = newsData.titulo;
        if (bodyEl && newsData.mensaje) bodyEl.textContent = newsData.mensaje;

        if (imgContainer && newsData.imagen) {
            // Replace SVG with Image
            imgContainer.innerHTML = `<img src="${newsData.imagen}" alt="News Icon" style="width: 100%; height: auto; max-width: 60px; border-radius: 8px;">`;
        }
    } catch (e) {
